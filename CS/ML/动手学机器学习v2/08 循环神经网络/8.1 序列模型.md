...
* *锚定*（anchoring）效应：基于其他人的意见做出评价。
  ...
  结果表明（ 文献（Wu.Ahmed.Beutel.ea.2017）），这种效应会使评分提高半个百分点以上。
...
简而言之，电影评分决不是固定不变的。
因此，使用时间动力学可以得到更准确的电影推荐 文献（Koren.2009）。
当然，序列数据不仅仅是关于电影评分的。
下面给出了更多的场景。
...
为了简单起见，我们以 图（fig_ftse100）所示的股票价格（富时100指数）为例。
...
例如在 图（sec_linear_concise）中训练的模型。
仅有一个主要问题：输入数据的数量，
...
第二种策略，如 图（fig_sequence-model）所示，
是保留一些对过去观测的总结$h_t$，
...
而反之则不行 文献（Hoyer.Janzing.Mooij.ea.2009）。
而这个向前推进的方向恰好也是我们通常感兴趣的方向。
彼得斯等人 文献（Peters.Janzing.Scholkopf.2017）
...
![](../img/generated/f847819f8e-output-1.svg)
...
现在，准备[**训练模型**]了。实现下面的训练代码的方式与前面几节（如 图（sec_linear_concise））中的循环训练基本相同。因此，我们不会深入探讨太多细节。
...
![](../img/generated/f847819f8e-output-2.svg)
...
![](../img/generated/f847819f8e-output-3.svg)
...
![](../img/generated/f847819f8e-output-4.svg)
