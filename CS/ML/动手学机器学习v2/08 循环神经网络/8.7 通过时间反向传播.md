...
输入表示为$x_t$，输出表示为$o_t$。
回想一下我们在 图（subsec_rnn_w_hidden_states）中的讨论，
...
在 公式（eq_bptt_partial_L_wh）中乘积的第一项和第二项很容易计算，
而第三项$\partial h_t/\partial w_h$是使事情变得棘手的地方，
因为我们需要循环地计算参数$w_h$对$h_t$的影响。
根据 公式（eq_bptt_ht_ot）中的递归计算，
...
公式 公式（eq_bptt_partial_ht_wh_recur）中的梯度计算
满足$a_{t}=b_{t}+c_{t}a_{t-1}$。
因此，对于每个 公式（eq_bptt_at），
我们可以使用下面的公式移除 公式（eq_bptt_partial_ht_wh_recur）中的循环计算
...
显然，我们可以仅仅计算 公式（eq_bptt_partial_ht_wh_gen）中的全部总和，
...
或者，我们可以在$\tau$步后截断
 公式（eq_bptt_partial_ht_wh_gen）中的求和计算。
这是我们到目前为止一直在讨论的内容，
例如在 图（sec_rnn_scratch）中分离梯度时。
这会带来真实梯度的*近似*，
只需将求和终止为$\partial h_{t-\tau}/\partial w_h$。
在实践中，这种方式工作得很好。
它通常被称为截断的通过时间反向传播 文献（Jaeger.2002）。
...
因此$E[\xi_t] = 1$。
我们使用它来替换 公式（eq_bptt_partial_ht_wh_recur）中的
...
所以将适当地加大权重。
这个想法是由塔莱克和奥利维尔 文献（Tallec.Ollivier.2017）提出的。

### 比较策略

![比较RNN中计算梯度的策略，3行自上而下分别为：随机截断、常规截断、完整计算](../img/truncated-bptt.svg)

 图（fig_truncated_bptt）说明了
当基于循环神经网络使用通过时间反向传播
...
与 图（subsec_bptt_analysis）中的分析不同，
...
由 公式（eq_bptt_partial_L_ot）给出的。

接下来，如 图（fig_rnn_bptt）所示，
...
我们可以从 公式（eq_bptt_partial_L_ht）中看到，
...
是由 公式（eq_bptt_partial_L_hT_final_step）和
 公式（eq_bptt_partial_L_ht_recur）递归计算得到的，
是影响数值稳定性的关键量。

正如我们在 图（sec_backprop）中所解释的那样，
