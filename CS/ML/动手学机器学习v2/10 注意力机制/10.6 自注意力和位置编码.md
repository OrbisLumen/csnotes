...
*自注意力*（self-attention）
 文献（Lin.Feng.Santos.ea.2017,Vaswani.Shazeer.Parmar.ea.2017），
也被称为*内部注意力*（intra-attention） 文献（Cheng.Dong.Lapata.2016,Parikh.Tackstrom.Das.ea.2016,Paulus.Xiong.Socher.2017）。
...
根据 公式（eq_attn-pooling）中定义的注意力汇聚函数$f$。
...
接下来比较下面几个架构，目标都是将由$n$个词元组成的序列映射到另一个长度相等的序列，其中的每个输入词元或输出词元都由$d$维向量表示。具体来说，将比较的是卷积神经网络、循环神经网络和自注意力这几个架构的计算复杂性、顺序操作和最大路径长度。请注意，顺序操作会妨碍并行计算，而任意的序列位置组合之间的路径越短，则能更轻松地学习序列中的远距离依赖关系 文献（Hochreiter.Bengio.Frasconi.ea.2001）。
...
接下来描述的是基于正弦函数和余弦函数的固定位置编码
 文献（Vaswani.Shazeer.Parmar.ea.2017）。
...
![](../img/generated/8fb3133c36-output-1.svg)
...
![](../img/generated/8fb3133c36-output-2.svg)
...
对于任何确定的位置偏移$\delta$，
 公式（eq_positional-encoding-def）中的任何一对
