...
本节将介绍*批量规范化*（batch normalization） 文献（Ioffe.Szegedy.2015），这是一种流行且有效的技术，可持续加速深层网络的收敛速度。
再结合在 图（sec_resnet）中将介绍的残差块，批量规范化使得研究人员能够训练100层以上的网络。
...
回想一下我们应用多层感知机来预测房价的例子（ 图（sec_kaggle_house））。
...
在 公式（eq_batchnorm）中，$\hat{\boldsymbol{\mu}}_\mathcal{B}$是小批量$\mathcal{B}$的样本均值，$\hat{\boldsymbol{\sigma}}_\mathcal{B}$是小批量$\mathcal{B}$的样本标准差。
...
从形式上来看，我们计算出 公式（eq_batchnorm）中的$\hat{\boldsymbol{\mu}}_\mathcal{B}$和${\hat{\boldsymbol{\sigma}}_\mathcal{B}}$，如下所示：
...
在一些初步研究中， 文献（Teye.Azizpour.Smith.2018）和 文献（Luo.Wang.Shao.ea.2018）分别将批量规范化的性质与贝叶斯先验相关联。
...
为了更好理解如何[**应用`BatchNorm`**]，下面我们将其应用(**于LeNet模型**)（ 图（sec_lenet））。
...
这个代码与我们第一次训练LeNet（ 图（sec_lenet））时几乎完全相同，主要区别在于学习率大得多。
...
![](../img/generated/7f142832a4-output-1.svg)
...
![](../img/generated/7f142832a4-output-2.svg)
...
他对该示例进行了详细回顾 文献（Lipton.Steinhardt.2018），概述了机器学习中令人不安的趋势。
此外，一些作者对批量规范化的成功提出了另一种解释：在某些方面，批量规范化的表现出与原始论文 文献（Santurkar.Tsipras.Ilyas.ea.2018）中声称的行为是相反的。
