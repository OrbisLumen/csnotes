...
*稠密连接网络*（DenseNet） 文献（Huang.Liu.Van-Der-Maaten.ea.2017）在某种程度上是ResNet的逻辑扩展。让我们先从数学上了解一下。
...
如 图（fig_densenet_block）所示，ResNet和DenseNet的关键区别在于，DenseNet输出是*连接*（用图中的$[,]$表示）而不是如ResNet的简单相加。
...
DenseNet使用了ResNet改良版的“批量规范化、激活和卷积”架构（参见 图（sec_resnet）中的练习）。
我们首先实现一下这个架构。
...
这里我们设成4，从而与 图（sec_resnet）的ResNet-18保持一致。
...
![](../img/generated/1384ddc0a7-output-1.svg)
...
1. 实现DenseNet论文 文献（Huang.Liu.Van-Der-Maaten.ea.2017）表1所示的不同DenseNet版本。
1. 应用DenseNet的思想设计一个基于多层感知机的模型。将其应用于 图（sec_kaggle_house）中的房价预测任务。
