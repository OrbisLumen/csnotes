...
在2014年的ImageNet图像识别挑战赛中，一个名叫*GoogLeNet* 文献（Szegedy.Liu.Jia.ea.2015）的网络架构大放异彩。
...
如 图（fig_inception）所示，Inception块由四条并行路径组成。
...
如 图（fig_inception_full）所示，GoogLeNet一共使用9个Inception块和全局平均汇聚层的堆叠来生成其估计值。Inception块之间的最大汇聚层可降低维度。
...
![](../img/generated/1767c0b2a6-output-1.svg)
...
1. GoogLeNet有一些后续版本。尝试实现并运行它们，然后观察实验结果。这些后续版本包括：
    * 添加批量规范化层 文献（Ioffe.Szegedy.2015）（batch normalization），在 图（sec_batch_norm）中将介绍；
    * 对Inception模块进行调整 文献（Szegedy.Vanhoucke.Ioffe.ea.2016）；
    * 使用标签平滑（label smoothing）进行模型正则化 文献（Szegedy.Vanhoucke.Ioffe.ea.2016）；
    * 加入残差连接 文献（Szegedy.Ioffe.Vanhoucke.ea.2017）。（ 图（sec_resnet）将介绍）。
