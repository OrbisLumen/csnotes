...
如 图（sec_semantic_segmentation）中所介绍的那样，语义分割是对图像中的每个像素分类。
*全卷积网络*（fully convolutional network，FCN）采用卷积神经网络实现了从图像像素到像素类别的变换 文献（Long.Shelhamer.Darrell.2015）。
与我们之前在图像分类或目标检测部分介绍的卷积神经网络不同，全卷积网络将中间层特征图的高和宽变换回输入图像的尺寸：这是通过在 图（sec_transposed_conv）中引入的*转置卷积*（transposed convolution）实现的。
...
如 图（fig_fcn）所示，全卷积网络先使用卷积神经网络抽取图像特征，然后通过$1\times 1$卷积层将通道数变换为类别个数，最后在 图（sec_transposed_conv）中通过转置卷积层将特征图的高和宽变换为输入图像的尺寸。
...
回想一下 图（sec_padding）中卷积层输出形状的计算方法：
...
除了坐标刻度不同，双线性插值放大的图像和在 图（sec_bbox）中打印出的原图看上去没什么两样。
...
![](../img/generated/15ebf413b1-output-1.svg)
...
我们用 图（sec_semantic_segmentation）中介绍的语义分割读取数据集。
...
![](../img/generated/15ebf413b1-output-2.svg)
...
![](../img/generated/15ebf413b1-output-3.svg)
...
1. 预测测试图像中所有像素的类别。
1. 最初的全卷积网络的论文中 文献（Long.Shelhamer.Darrell.2015）还使用了某些卷积神经网络中间层的输出。试着实现这个想法。
