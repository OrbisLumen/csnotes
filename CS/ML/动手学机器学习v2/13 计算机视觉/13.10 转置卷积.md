...
到目前为止，我们所见到的卷积神经网络层，例如卷积层（ 图（sec_conv_layer））和汇聚层（ 图（sec_pooling）），通常会减少下采样输入图像的空间维度（高和宽）。
...
本节将介绍
*转置卷积*（transposed convolution） 文献（Dumoulin.Visin.2016），
用于逆转下采样导致的空间尺寸减小。
...
例如， 图（fig_trans_conv）解释了如何为$2\times 2$的输入张量计算卷积核为$2\times 2$的转置卷积。
...
与通过卷积核“减少”输入元素的常规卷积（在 图（sec_conv_layer）中）相比，转置卷积通过卷积核“广播”输入元素，从而产生大于输入的输出。
我们可以通过 图（fig_trans_conv）来构建输入张量`X`和卷积核张量`K`从而[**验证上述实现输出**]。
此实现是基本的二维转置卷积运算。
...
使用 图（fig_trans_conv）中相同输入和卷积核张量，将步幅从1更改为2会增加中间张量的高和权重，因此输出张量在 图（fig_trans_conv_stride2）中。

![卷积核为$2\times 2$，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。](../img/trans_conv_stride2.svg)

以下代码可以验证 图（fig_trans_conv_stride2）中步幅为2的转置卷积的输出。
...
1. 在 图（subsec-connection-to-mat-transposition）中，卷积输入`X`和转置的卷积输出`Z`具有相同的形状。他们的数值也相同吗？为什么？
