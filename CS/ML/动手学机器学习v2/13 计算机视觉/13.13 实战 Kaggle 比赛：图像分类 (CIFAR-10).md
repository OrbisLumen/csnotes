...
我们在 图（sec_image_augmentation）中对CIFAR-10数据集做了一个实验。CIFAR-10是计算机视觉领域中的一个重要的数据集。
本节将运用我们在前几节中学到的知识来参加CIFAR-10图像分类问题的Kaggle竞赛，(**比赛的网址是https://www.kaggle.com/c/cifar-10**)。

 图（fig_kaggle_cifar10）显示了竞赛网站页面上的信息。
...
这些图片共涵盖10个类别：飞机、汽车、鸟类、猫、鹿、狗、青蛙、马、船和卡车。
 图（fig_kaggle_cifar10）的左上角显示了数据集中飞机、汽车和鸟类的一些图像。

### 下载数据集

登录Kaggle后，我们可以点击 图（fig_kaggle_cifar10）中显示的CIFAR-10图像分类竞赛网页上的“Data”选项卡，然后单击“Download All”按钮下载数据集。
...
我们定义了 图（sec_resnet）中描述的Resnet-18模型。
...
![](../img/generated/8a33ba13f9-output-1.svg)
...
![](../img/generated/8a33ba13f9-output-2.svg)

向Kaggle提交结果的方法与 图（sec_kaggle_house）中的方法类似，上面的代码将生成一个
