...
回想一下，之前我们将softmax回归模型（ 图（sec_softmax_scratch））和多层感知机模型（ 图（sec_mlp_scratch））应用于Fashion-MNIST数据集中的服装图片。
...
这个模型是由AT&T贝尔实验室的研究员Yann LeCun在1989年提出的（并以其命名），目的是识别图像 文献（LeCun.Bottou.Bengio.ea.1998）中的手写数字。
...
下面，我们将一个大小为$28 \times 28$的单通道（黑白）图像通过LeNet。通过在每一层打印输出的形状，我们可以[**检查模型**]，以确保其操作与我们期望的 图（img_lenet_vert）一致。
...
为了进行评估，我们需要[**对**] 图（sec_softmax_scratch）中描述的(**`evaluate_accuracy`函数进行轻微的修改**)。
...
[**为了使用GPU，我们还需要一点小改动**]。
与 图（sec_softmax_scratch）中定义的`train_epoch_ch3`不同，在进行正向和反向传播之前，我们需要将每一小批量数据移动到我们指定的设备（例如GPU）上。

如下所示，训练函数`train_ch6`也类似于 图（sec_softmax_scratch）中定义的`train_ch3`。
由于我们将实现多层神经网络，因此我们将主要使用高级API。
以下训练函数假定从高级API创建的模型作为输入，并进行相应的优化。
我们使用在 图（subsec_xavier）中介绍的Xavier随机初始化模型参数。
...
![](../img/generated/04af3cf370-output-1.svg)
