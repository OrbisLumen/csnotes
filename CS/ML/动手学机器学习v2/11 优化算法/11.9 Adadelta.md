...
Adadelta是AdaGrad的另一种变体（ 图（sec_adagrad）），
主要区别在于前者减少了学习率适应坐标的数量。
此外，广义上Adadelta被称为没有学习率，因为它使用变化量本身作为未来变化的校准。
Adadelta算法是在 文献（Zeiler.2012）中提出的。
...
以下是Adadelta的技术细节。鉴于参数du jour是$\rho$，我们获得了与 图（sec_rmsprop）类似的以下泄漏更新：
...
与 图（sec_rmsprop）的区别在于，我们使用重新缩放的梯度$\mathbf{g}_t'$执行更新，即
...
![](../img/generated/daed88884a-output-1.svg)
...
![](../img/generated/daed88884a-output-2.svg)
