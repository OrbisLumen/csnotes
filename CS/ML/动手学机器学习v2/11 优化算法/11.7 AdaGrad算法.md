...
AdaGrad算法 文献（Duchi.Hazan.Singer.2011）通过将粗略的计数器$s(i, t)$替换为先前观察所得梯度的平方之和来解决这个问题。
...
正如在 图（sec_momentum）中那样，我们可以根据其特征分解$\mathbf{Q} = \mathbf{U}^\top \boldsymbol{\Lambda} \mathbf{U}$重写这个问题，来得到一个简化得多的问题，使每个坐标都可以单独解出：
...
详尽的分析（要花几页解释）超出了本节的范围，请读者参考 文献（Duchi.Hazan.Singer.2011）。
...
![](../img/generated/c9b20e5c9a-output-1.svg)
...
![](../img/generated/c9b20e5c9a-output-2.svg)
...
与 图（sec_minibatch_sgd）一节中的实验相比，这里使用更大的学习率来训练模型。
...
![](../img/generated/c9b20e5c9a-output-3.svg)
...
![](../img/generated/c9b20e5c9a-output-4.svg)
...
* 在深度学习问题上，AdaGrad算法有时在降低学习率方面可能过于剧烈。我们将在 图（sec_adam）一节讨论缓解这种情况的策略。
...
1. 尝试对适当的深度网络使用AdaGrad算法，例如，图（sec_lenet）中应用于Fashion-MNIST的深度网络。
