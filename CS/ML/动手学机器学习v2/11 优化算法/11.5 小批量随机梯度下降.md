...
到目前为止，我们在基于梯度的学习方法中遇到了两个极端情况：
 图（sec_gd）中使用完整数据集来计算梯度并更新参数，
 图（sec_sgd）中一次处理一个训练样本来取得进展。
...
需要注意的是，在 图（sec_batch_norm）中，我们使用了一种在很大程度上取决于小批量中的方差的正则化。
随着后者增加，方差会减少，随之而来的是批量规范化带来的噪声注入的好处。
关于实例，请参阅 文献（Ioffe.2017），了解有关如何重新缩放并计算适当项目。
...
## 从零开始实现
 ...
 图（sec_linear_scratch）一节中已经实现过小批量随机梯度下降算法。
...
![](../img/generated/aaaa08a1bb-output-1.svg)
...
![](../img/generated/aaaa08a1bb-output-2.svg)
...
![](../img/generated/aaaa08a1bb-output-3.svg)
...
![](../img/generated/aaaa08a1bb-output-4.svg)
...
![](../img/generated/aaaa08a1bb-output-5.svg)
...
![](../img/generated/aaaa08a1bb-output-6.svg)
