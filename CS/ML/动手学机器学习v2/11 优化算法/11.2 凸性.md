...
这可能会产生一些像 文献（Izmailov.Podoprikhin.Garipov.ea.2018）这样比较有意思的新优化变体。
...
在 图（fig_nonconvex）中连接$a$和$b$的线段需要包含一部分既不在$\mathcal{X}$也不在$\mathcal{Y}$中。
...
![](../img/generated/2d898f5132-output-1.svg)
...
![](../img/generated/2d898f5132-output-2.svg)
...
事实上，我们一直在使用这个技巧。
比如权重衰减 图（sec_weight_decay），在目标函数中加入$\frac{\lambda}{2} |\mathbf{w}|^2$，以确保$\mathbf{w}$不会增长太大。
...
同样，我们之前也遇到过，例如在 图（sec_rnn_scratch）中处理梯度截断时，我们通过
...
投影的数学定义听起来可能有点抽象，为了解释得更清楚一些，请看 图（fig_projections）。
图中有两个凸集，一个圆和一个菱形。
...
这是 图（fig_projections）中菱形例子的一个广义版本。
