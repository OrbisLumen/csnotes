...
如 图（fig_circle_area）所示，内接多边形的等长边越多，就越接近圆。
...
正如在 图（subsec_norms_and_objectives）中讨论的那样，
这种问题在深度学习中是无处不在的。
...
我们可以将 公式（eq_derivative）中的导数$f'(x)$解释为$f(x)$相对于$x$的*瞬时*（instantaneous）变化率。
...
[**通过令$x=1$并让$h$接近$0$，**] 公式（eq_derivative）中(**$\frac{f(x+h)-f(x)}{h}$的数值结果接近$2$**)。
...
![](../img/generated/5138d3192e-output-1.svg)
