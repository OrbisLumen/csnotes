...
我们已经在 图（sec_mlp）中描述了多层感知机（MLP），
现在让我们尝试自己实现一个多层感知机。
为了与之前softmax回归（ 图（sec_softmax_scratch） ）
获得的结果进行比较，
...
由于我们已经从零实现过softmax函数（ 图（sec_softmax_scratch）），
因此在这里我们直接使用高级API中的内置函数来计算softmax和交叉熵损失。
回想一下我们之前在 图（subsec_softmax-implementation-revisited）中
对这些复杂问题的讨论。
...
可以直接调用`d2l`包的`train_ch3`函数（参见 图（sec_softmax_scratch） ），
...
![](../img/generated/66f734defc-output-1.svg)
...
![](../img/generated/66f734defc-output-2.svg)
