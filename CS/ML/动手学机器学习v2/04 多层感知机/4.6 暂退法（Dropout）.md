...
简单性以较小维度的形式展现，
我们在 图（sec_model_selection） 讨论线性模型的单项式函数时探讨了这一点。
此外，正如我们在 图（sec_weight_decay） 中讨论权重衰减（$L_2$正则化）时看到的那样，
参数的范数也代表了一种有用的简单性度量。

简单性的另一个角度是平滑性，即函数不应该对其输入的微小变化敏感。
例如，当我们对图像进行分类时，我们预计向像素添加一些随机噪声应该是基本无影响的。
1995年，克里斯托弗·毕晓普证明了
具有输入噪声的训练等价于Tikhonov正则化 文献（Bishop.1995）。
这项工作用数学证实了“要求函数光滑”和“要求函数对输入的随机噪声具有适应性”之间的联系。

然后在2014年，斯里瓦斯塔瓦等人 文献（Srivastava.Hinton.Krizhevsky.ea.2014）
...
回想一下 图（fig_mlp）中带有1个隐藏层和5个隐藏单元的多层感知机。
当我们将暂退法应用到隐藏层，以$p$的概率将隐藏单元置为零时，
结果可以看作一个只包含原始神经元子集的网络。
比如在 图（fig_dropout2）中，删除了$h_2$和$h_5$，
...
同样，我们使用 图（sec_fashion_mnist）中引入的Fashion-MNIST数据集。
...
![](../img/generated/1902eb4678-output-1.svg)
...
![](../img/generated/1902eb4678-output-2.svg)
