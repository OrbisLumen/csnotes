...
此数据集由Bart de Cock于2011年收集 文献（De-Cock.2011），
...
如果我们想参加Kaggle比赛，首先需要注册一个账户（见 图（fig_kaggle））。

![Kaggle网站](../img/kaggle.png)

在房价预测比赛页面（如 图（fig_house_pricing） 所示）的"Data"选项卡下可以找到数据集。我们可以通过下面的网址提交预测，并查看排名：
...
我们只能在官方测试集中评估我们的模型。
在 图（fig_house_pricing） 中，"Data"选项卡有下载数据的链接。

开始之前，我们将[**使用`pandas`读入并处理数据**]，
这是我们在 图（sec_pandas）中引入的。
...
（请参见 图（subsec_classification-problem））。
...
本书在讨论模型选择的部分（ 图（sec_model_selection））
中介绍了[**K折交叉验证**]，
它有助于模型选择和超参数调整。
...
![](../img/generated/ae47e35c67-output-1.svg)
...
![](../img/generated/ae47e35c67-output-2.svg)

接下来，如 图（fig_kaggle_submit2）中所示，
