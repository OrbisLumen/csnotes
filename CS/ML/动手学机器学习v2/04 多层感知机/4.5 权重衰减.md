...
回想一下，在多项式回归的例子（ 图（sec_model_selection））中，
...
在 图（subsec_lin-algebra-norms）中，
...
这正是我们想要的。
让我们回顾一下 图（sec_linear_regression）中的线性回归例子。
我们的损失由下式给出：
...
使用与 公式（eq_linreg_batch_update）中的相同符号，
...
从 图（chap_linear）以来，线性网络和平方损失没有变化，
...
![](../img/generated/eab93bd2a9-output-1.svg)
...
![](../img/generated/eab93bd2a9-output-2.svg)
...
![](../img/generated/eab93bd2a9-output-3.svg)
...
![](../img/generated/eab93bd2a9-output-4.svg)
...
1. 我们知道$\|\mathbf{w}\|^2 = \mathbf{w}^\top \mathbf{w}$。能找到类似的矩阵方程吗（见 图（subsec_lin-algebra-norms） 中的Frobenius范数）？
