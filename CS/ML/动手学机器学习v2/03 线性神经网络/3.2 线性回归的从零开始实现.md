...
![](../img/generated/65191dad4b-output-1.svg)
...
我们使用 图（sec_autograd）中引入的自动微分来计算梯度。
...
回想一下 图（subsec_broadcasting）中描述的广播机制：
...
这里我们使用 图（sec_linear_regression）中描述的平方损失函数。
...
正如我们在 图（sec_linear_regression）中讨论的，线性回归有解析解。
...
我们现在忽略这些细节，以后会在 图（chap_optimization）中详细介绍。
