...
本节我们将使用刚刚在 图（sec_fashion_mnist）中引入的Fashion-MNIST数据集，
并设置数据迭代器的批量大小为256。
...
如 图（subseq_lin-alg-reduction）和
 图（subseq_lin-alg-non-reduction）所述，
...
接下来，我们实现 图（sec_softmax）中引入的交叉熵损失函数。
...
在我们看过 图（sec_linear_scratch）中的线性回归实现，
...
作为一个从零开始的实现，我们使用 图（sec_linear_scratch）中定义的
...
![](../img/generated/fd51072bbf-output-1.svg)
...
![](../img/generated/fd51072bbf-output-2.svg)
