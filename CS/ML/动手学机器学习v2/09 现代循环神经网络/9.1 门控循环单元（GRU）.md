...
其中最早的方法是"长短期记忆"（long-short-term memory，LSTM）
 文献（Hochreiter.Schmidhuber.1997），
我们将在 图（sec_lstm）中讨论。
门控循环单元（gated recurrent unit，GRU）
 文献（Cho.Van-Merrienboer.Bahdanau.ea.2014）
是一个稍微简化的变体，通常能够提供同等的效果，
并且计算 文献（Chung.Gulcehre.Cho.ea.2014）的速度明显更快。
...
请注意，在求和过程中会触发广播机制
（请参阅 图（subsec_broadcasting））。
我们使用sigmoid函数（如 图（sec_mlp）中介绍的）
将输入值转换到区间$(0, 1)$。

### 候选隐状态

接下来，让我们将重置门$\mathbf{R}_t$
与 公式（rnn_h_with_state）
中的常规隐状态更新机制集成，
...
与 公式（rnn_h_with_state）相比，
 公式（gru_tilde_H）中的$\mathbf{R}_t$和$\mathbf{H}_{t-1}$
的元素相乘可以减少以往状态的影响。
每当重置门$\mathbf{R}_t$中的项接近$1$时，
我们恢复一个如 公式（rnn_h_with_state）中的普通的循环神经网络。
对于重置门$\mathbf{R}_t$中所有接近$0$的项，
候选隐状态是以$\mathbf{X}_t$作为输入的多层感知机的结果。
因此，任何预先存在的隐状态都会被*重置*为默认值。

 图（fig_gru_2）说明了应用重置门之后的计算流程。
...
首先，我们读取 图（sec_rnn_scratch）中使用的时间机器数据集：
...
与 图（sec_rnn_scratch）中定义的`init_rnn_state`函数一样，
...
训练和预测的工作方式与 图（sec_rnn_scratch）完全相同。
...
![](../img/generated/873028a405-output-1.svg)
...
![](../img/generated/873028a405-output-2.svg)
