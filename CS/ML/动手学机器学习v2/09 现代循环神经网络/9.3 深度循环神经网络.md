...
而微观数据只记录较短期的时间动态。
 ...
 图（fig_deep_rnn）描述了一个具有$L$个隐藏层的深度循环神经网络，
...
这个架构是由 图（fig_deep_rnn）中描述了$L$个隐藏层构成。
...
来代替 公式（eq_deep_rnn_H）中的隐状态进行计算，
...
像选择超参数这类架构决策也跟 图（sec_lstm）中的决策非常相似。
...
![](../img/generated/2a71983c11-output-1.svg)
...
1. 基于我们在 图（sec_rnn_scratch）中讨论的单层实现，
   尝试从零开始实现两层循环神经网络。
