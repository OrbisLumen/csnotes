...
它也是通用消息传递算法 文献（Aji.McEliece.2000）的一个非常特殊的例子。
结合前向和后向递归，我们能够计算
...
有关更多详细信息，请参阅 文献（Doucet.De-Freitas.Gordon.2001）。
...
添加了反向传递信息的隐藏层，以便更灵活地处理此类信息。
 图（fig_birnn）描述了具有单个隐藏层的双向循环神经网络的架构。
...
双向循环神经网络是由 文献（Schuster.Paliwal.1997）提出的，
关于各种架构的详细讨论请参阅 文献（Graves.Schmidhuber.2005）。
让我们看看这样一个网络的细节。
...
在 图（sec_bert）和 图（sec_sentiment_rnn）中，
...
![](../img/generated/04850629fc-output-1.svg)

上述结果显然令人瞠目结舌。
关于如何更有效地使用双向循环神经网络的讨论，
请参阅 图（sec_sentiment_rnn）中的情感分类应用。
