...
因此我们将其做为本章剩余部分和 图（chap_attention）的重点。
...
统计学方法在这一领域一直占据主导地位
 文献（Brown.Cocke.Della-Pietra.ea.1988,Brown.Cocke.Della-Pietra.ea.1990）。
...
与 图（sec_language_model）中的字符级词元化不同，
在机器翻译中，我们更喜欢单词级词元化
（最先进的模型可能使用更高级的词元化技术）。
...
![](../img/generated/a221958f60-output-1.svg)
...
这个固定长度是由 图（sec_language_model）中的
